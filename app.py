...
def products():
    ...
    categories_filter = ','.join(selected_categories) if selected_categories else None
    brands_filter = ','.join(selected_brands) if selected_brands else None
    tags_filter = ','.join(selected_tags) if selected_tags else None

    try:
        # Fetch filtered products
        cursor.execute(
            "EXEC GetFilteredProducts @Categories=?, @Brands=?, @Tags=?, @Page=?, @PerPage=?",
            (categories_filter, brands_filter, tags_filter, page, per_page)
        )
        ...
        cursor.execute(
            "EXEC GetTotalProducts @Categories=?, @Brands=?, @Tags=?", 
            (categories_filter, brands_filter, tags_filter)
        )
...
def product_details(product_id):
    ...
    try:
        # Выполняем хранимую процедуру для получения деталей продукта
        cursor.execute("EXEC GetProductDetails ?", (product_id,))
        result = cursor.fetchone()
        print("Product Details:", result)  # Логируем результат
        ...
        product = {
            ...
            "BrandName": result[7],
            "Tags": result[8],
            "Stock": result[9],  # Добавляем Stock
            "Reserved": result[10],  # Добавляем Reserved
        }
        print("Parsed Product Data:", product)  # Логируем структуру данных

        # Получаем отзывы о продукте
        cursor.execute("EXEC GetProductReviews ?", (product_id,))
        reviews = cursor.fetchall()
        print("Product Reviews:", reviews)  # Логируем отзывы
        ...
        flash(f"Error while fetching product details: {str(e)}", 'danger')
